...
export function TrendChart({ data, dataKey = 'value', color = '#06B6D4', unit = '', area = true, yDomain }) {
  ...
  return (
    <Comp data={data} margin={{ top: 8, right: 12, left: -14, bottom: 0 }}>
      <defs>
...
export function DualTrendChart({ data, leftKey = 'count', rightKey = 'downtime' }) {
  return (
    <ComposedChart data={data} margin={{ top: 8, right: 8, left: -14, bottom: 0 }}>
      ...
      <YAxis yAxisId="right" orientation="right" tick={AXIS} axisLine={false} tickLine={false} width={44} />
      <ChartTooltip />
...
export function HorizontalBarChart({ data, dataKey = 'count', color = '#06B6D4', unit = '' }) {
  return (
    <BarChart data={data} layout="vertical" margin={{ top: 4, right: 24, left: 8, bottom: 0 }} barCategoryGap="30%">
      ...
      <YAxis type="category" dataKey="label" tick={{ ...AXIS, fill: '#94A3B8' }} axisLine={false} tickLine={false} width={130} />
      <ChartTooltip formatter={(v) => [`${v}${unit}`, null]} />
...
export function ParetoChart({ data }) {
  return (
    <ComposedChart data={data} margin={{ top: 8, right: 6, left: -14, bottom: 34 }}>
      <CartesianGrid stroke={GRID} vertical={false} />
...
export function GroupedBarChart({ data, bars }) {
  return (
    <BarChart data={data} margin={{ top: 8, right: 12, left: -14, bottom: 0 }} barCategoryGap="24%">
      <CartesianGrid stroke={GRID} vertical={false} />
      <XAxis dataKey="label" tick={AXIS} axisLine={false} tickLine={false} />
